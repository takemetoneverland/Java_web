...
public class EmployeeManager {
	...
	public static void main(String[] args) {
		...
		//실제로 입력된 데이터(사운의 정보)의 개수를 체크하는 변수.
		int count =  0;
			...
			if(menu == 1) {
				...
				while(true) {
					...
					for(int i=0; i<count; i++) {
						if(empNum.equals(userNums[i])) { //입력한 사번이 이미 존재한느 경우
				...
				//입력한 사번에 일치하는 사원의 4가지 정보를 모두 추력하세요.
				...
				for(int i=0; i<count; i++) {
					if(empNum.equals(userNums[i])) {
						...
						case "Y": case "y": case "ㅛ":
							for(int j=i; j<count-1; j++) { //맨 뒤에 값은 땡겨올 값이 없으니까.
