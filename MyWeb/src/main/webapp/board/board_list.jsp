	...
	<%--
	 ...
	 <c:if test="${user == null}">
	 	<script>
	 		...
	 		// 컨트롤러를 기준으로 상대 경로로 작성하거나, 절대경로를 사용한다.
			...
			<tbody>
				<c:forEach var="b" items="${boardList}">
					<tr>
						...
						<td>${b.writer}</td>
						<td>
							<a href="/MyWeb/content.board?bId=${b.bId}&page=${param.page}&countPerPage=${param.countPerPage}">${b.title}</a>
							&nbsp;&nbsp;
							<c:if test="${b.newMark}">
						...
						<td>
							<fmt:formatDate value="${b.date}" pattern="yyyy년 MM월 dd일 a hh시  mm분"/>
						</td>
						<td>${b.hit}</td>
					</tr>
				</c:forEach>
